  ...
  try {
    ...
    return new Response(pdf, {
      headers: {
        ...
        "Content-Disposition": `inline; filename="${getViajeStudentsReportFileName(data)}"`,
        "Content-Type": "application/pdf",
      },
    });
  } catch (error) {
    console.error("[viaje-report-route] failed to generate report", error);
    ...
    return Response.json(
      {
        message:
          error instanceof Error
            ? error.message
            : "No se pudo generar el reporte del viaje.",
      },
      { status: 500 },
    );
  }

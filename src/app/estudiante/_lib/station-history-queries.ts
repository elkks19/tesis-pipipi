...
function hasCompletedStation(doc: HistoriaDocument, key: ComplementaryExamKey) {
  return Boolean(doc[key]);
}

async function serializeHistoria(doc: HistoriaDocument, key: ComplementaryExamKey) {
  const isRequested = Boolean(doc.examenesComplementariosSolicitados?.[key]);

  if (
    !doc._id ||
    doc.type !== "historia" ||
    !isRequested ||
    hasCompletedStation(doc, key)
  ) {
    return null;
  }
...
export async function listPendingComplementaryExamHistories({
  ...
  while (rows.length <= PAGE_SIZE && !reachedEnd) {
    ...
    for (const resultRow of result.rows) {
      ...
      const row = await serializeHistoria(doc, examKey);

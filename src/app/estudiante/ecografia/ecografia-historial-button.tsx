...
import { getEcografiaHistorial } from "./historial-actions";

type SummaryData = Awaited<ReturnType<typeof getEcografiaHistorial>>;

export function EcografiaHistorialButton({ historiaId }: { historiaId: string }) {
  ...
  async function openHistorial() {
    ...
    setLoading(true);
    try {
      const summary = await getEcografiaHistorial(historiaId);
      setData(summary);
      setOpen(true);
    } catch (error) {

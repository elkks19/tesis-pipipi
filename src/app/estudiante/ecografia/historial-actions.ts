...
export async function getEcografiaHistorial(historiaId: string) {
  ...
  if (!isHistoriaDocument(doc) ||
      doc.examenesComplementariosSolicitados?.ecografia !== true ||
      !await canAccessActiveStationHistoria({
        historia: doc,
        stationKey: "ecografia",
        userId,
      })) {
    throw new Error("No se encontró una historia derivada a ecografía en tu viaje activo.");
  }

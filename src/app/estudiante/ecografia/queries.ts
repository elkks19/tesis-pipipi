...
export type EcografiaRow = {
  historiaId: string;
  paciente: PacienteSearchResult;
};

export type EcografiaPageResult = {
  hasNextPage: boolean;
  nextCursor?: string;
  pageSize: number;
  rows: EcografiaRow[];
...
function rowMatchesQuery(row: EcografiaRow, query: string) {
  if (!query) {
    return true;
  }
...
async function serializeHistoria(doc: HistoriaDocument) {
  const ecografiaSolicitada = Boolean(
    doc.examenesComplementariosSolicitados?.ecografia,
  );

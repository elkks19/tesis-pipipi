...
export async function getElectrocardiogramaHistorial(historiaId: string) {
  ...
  if (!isHistoriaDocument(doc) ||
      doc.examenesComplementariosSolicitados?.electrocardiograma !== true ||
      !await canAccessActiveStationHistoria({
        historia: doc,
        stationKey: "electrocardiograma",
        userId,
      })) {
    throw new Error("No se encontró una historia derivada a electrocardiograma en tu viaje activo.");
  }

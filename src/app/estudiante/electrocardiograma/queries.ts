...
export type ElectrocardiogramaRow = {
  historiaId: string;
  paciente: PacienteSearchResult;
};

export type ElectrocardiogramaPageResult = {
  hasNextPage: boolean;
  nextCursor?: string;
  pageSize: number;
  rows: ElectrocardiogramaRow[];
};
...
function rowMatchesQuery(row: ElectrocardiogramaRow, query: string) {
  if (!query) {
    return true;
  }
...
async function serializeHistoria(doc: HistoriaDocument) {
  const electrocardiogramaSolicitado = Boolean(
    doc.examenesComplementariosSolicitados?.electrocardiograma,
  );

  if (
    !doc._id ||
    doc.type !== "historia" ||
    !electrocardiogramaSolicitado ||
    Boolean(doc.electrocardiograma)
  ) {
    return null;
  }
...
export async function listHistoriasForElectrocardiograma({
  ...
  const rows: ElectrocardiogramaRow[] = [];

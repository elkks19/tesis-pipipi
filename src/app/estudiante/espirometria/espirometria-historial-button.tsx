...
import { getEspirometriaHistorial } from "./historial-actions";

type SummaryData = Awaited<ReturnType<typeof getEspirometriaHistorial>>;
type View = "historial" | "clinico";

export function EspirometriaHistorialButton({
  hasClinicalDetail = false,
  historiaId,
  showPdf = false,
}: {
  ...
  async function openView(nextView: View) {
    ...
    setLoading(nextView);
    try {
      const summary = await getEspirometriaHistorial(historiaId);
      setData(summary);
      setView(nextView);
      setOpen(true);
    } catch (error) {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {showPdf ? (
          <Button asChild size="sm" variant="outline"><a href={`/reportes/historias/${encodeURIComponent(historiaId)}?download=1`}><DownloadIcon data-icon="inline-start" />Descargar PDF</a></Button>
        ) : null}
      </div>
      {data ? (

...
import { getExamenGeneralSummary } from "./summary-actions";

type SummaryData = Awaited<ReturnType<typeof getExamenGeneralSummary>>;
type View = "historial" | "clinico";

export function ExamenGeneralQuickActions({
  hasClinicalDetail,
  historiaId,
  showPdf = false,
}: {
  ...
  async function openView(nextView: View) {
    ...
    setLoading(nextView);
    try {
      const summary = await getExamenGeneralSummary(historiaId);
      setData(summary);
      setView(nextView);
      setOpen(true);
    } catch (error) {

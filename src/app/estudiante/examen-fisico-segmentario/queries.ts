...
export type ExamenFisicoSegmentarioRow = {
  historiaId: string;
  paciente: PacienteSearchResult;
};

export type ExamenFisicoSegmentarioPageResult = {
  hasNextPage: boolean;
  nextCursor?: string;
  pageSize: number;
  rows: ExamenFisicoSegmentarioRow[];
};
...
function rowMatchesQuery(row: ExamenFisicoSegmentarioRow, query: string) {
  if (!query) {
    return true;
  }
...
async function serializeHistoria(doc: HistoriaDocument) {
  if (!doc._id || doc.type !== "historia" || doc.examenFisicoSegmentario) {
    return null;
  }
...
export async function listHistoriasForExamenFisicoSegmentario({
  ...
  const rows: ExamenFisicoSegmentarioRow[] = [];

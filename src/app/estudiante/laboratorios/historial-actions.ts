...
export async function getLaboratoriosHistorial(historiaId: string) {
  ...
  if (!isHistoriaDocument(doc) ||
      doc.examenesComplementariosSolicitados?.laboratorios !== true ||
      !await canAccessActiveStationHistoria({
        historia: doc,
        stationKey: "laboratorios",
        userId,
      })) {
    throw new Error("No se encontró una historia derivada a laboratorios en tu viaje activo.");
  }
